	...
	<view class="detail-shell">
		<view class="detail-content">
			<view class="detail-header">
				...
				<button v-if="cycle && !cycle.archived" class="detail-more" :disabled="mutationBusy" aria-label="停用周期项目" @tap="confirmArchive"><view></view><view></view><view></view></button>
			...
			<template v-else-if="cycle">
				...
				<view class="detail-calendar-card">
					...
					<view class="calendar-grid">
						<view v-for="day in calendarDays" :key="day.key" class="calendar-day" :class="{ muted: !day.currentMonth, today: day.value === currentDate, completed: cycle.completions.includes(day.value) }"><text>{{ day.day }}</text></view>
					</view>
				...
				<view v-else class="history-list">
					<view v-for="row in historyRows" :key="row.date" class="history-row">
						<view class="history-line"><view class="history-dot"></view><view v-if="!row.last" class="history-connector"></view></view>
						<view class="history-copy"><text>{{ formatFullDate(row.date) }}</text><text>{{ row.interval === null ? '首次打卡' : `距上次 ${row.interval} 天` }}</text></view>
						<text class="history-order">第 {{ row.order }} 次</text>
					</view>
				</view>
			</template>
		</view>
...
<script>
	...
	import { queueCycleFeedback, showCycleFeedback } from '../../modules/cycle/utils/cycle-feedback.js'

	export default {
		data() {
			return { cycleId: '', cycle: null, loading: true, mutationBusy: false, error: '', currentDate: localDateString(), calendarCursor: localDateString(), weekdays: ['一', '二', '三', '四', '五', '六', '日'], hasLoaded: false }
		},
		computed: {
			...
			historyRows() {
				...
				return ascending.map((date, index) => ({ date, order: index + 1, interval: index ? daysBetween(ascending[index - 1], date) : null })).reverse().map((row, index, rows) => ({ ...row, last: index === rows.length - 1 }))
			},
			averageIntervalText() {
		...
		methods: {
			...
			async archiveCycle() {
				...
				try { await cycleDataService.archiveCycle({ id: this.cycle._id }); queueCycleFeedback('已停用'); uni.navigateBack() }
...
<style>
	...
	.detail-stats-card { display:grid; grid-template-columns:1fr 140rpx 1fr; align-items:center; min-height:148rpx; margin-bottom:20rpx; padding:20rpx 22rpx; border:1rpx solid rgba(91,76,133,.1); border-radius:27rpx; background:rgba(255,255,255,.91); box-shadow:0 10rpx 28rpx rgba(58,51,93,.05); }.detail-stat-side { display:flex; min-width:0; flex-direction:column; }.detail-stat-side.right { text-align:right; }.detail-stat-side text:first-child { margin-bottom:8rpx; color:#9897a7; font-size:18rpx; }.detail-stat-side text:last-child { color:#4e4f5e; font-size:19rpx; line-height:29rpx; }.detail-days-stat { display:flex; align-items:center; flex-direction:column; justify-content:center; border-right:1rpx solid #e7e3ee; border-left:1rpx solid #e7e3ee; color:#9a99a9; font-size:17rpx; }.detail-days-stat>view { display:flex; align-items:baseline; }.detail-days-number { color:#7046e8; font-size:45rpx; font-weight:790; line-height:50rpx; }.detail-days-unit { margin-left:4rpx; color:#7046e8; font-size:19rpx; font-weight:650; }.detail-no-record { color:#777687; font-size:21rpx; font-weight:650; line-height:50rpx; }
	.detail-calendar-card { margin-bottom:28rpx; padding:24rpx 22rpx 20rpx; border:1rpx solid rgba(91,76,133,.1); border-radius:28rpx; background:rgba(255,255,255,.92); box-shadow:0 10rpx 28rpx rgba(58,51,93,.05); }.calendar-header { display:flex; align-items:center; justify-content:space-between; margin-bottom:18rpx; }.calendar-header>text { color:#353643; font-size:25rpx; font-weight:700; }.month-button { display:flex; align-items:center; justify-content:center; width:56rpx; height:56rpx; margin:0; padding:0; border:0 !important; border-radius:17rpx; background:#f4f2f8 !important; }.month-button::after { border:0 !important; }.month-chevron { width:13rpx; height:13rpx; border-top:3rpx solid #737282; border-right:3rpx solid #737282; }.month-chevron.left { transform:rotate(-135deg); }.month-chevron.right { transform:rotate(45deg); }.calendar-weekdays,.calendar-grid { display:grid; grid-template-columns:repeat(7,1fr); }.calendar-weekdays { margin-bottom:7rpx; color:#aaa8b5; font-size:17rpx; text-align:center; }.calendar-day { position:relative; display:flex; align-items:center; justify-content:center; height:67rpx; color:#575866; font-size:20rpx; }.calendar-day>text { display:flex; align-items:center; justify-content:center; width:48rpx; height:48rpx; border:4rpx solid transparent; border-radius:50%; }.calendar-day.muted { color:#c2c0ca; }.calendar-day.today>text { background:#eee9fb; color:#7046e8; font-weight:700; }.calendar-day.completed>text { border-color:#7046e8; background:transparent; color:#6037d9; font-weight:700; }.calendar-legend { display:flex; align-items:center; justify-content:center; gap:8rpx; margin-top:11rpx; color:#9695a5; font-size:17rpx; }.calendar-legend view { width:17rpx; height:17rpx; border:3rpx solid #7046e8; border-radius:50%; }
	...
	.history-list { overflow:hidden; padding:7rpx 20rpx; border:1rpx solid rgba(91,76,133,.1); border-radius:25rpx; background:rgba(255,255,255,.91); }.history-row { display:flex; min-height:92rpx; align-items:center; }.history-line { position:relative; display:flex; align-self:stretch; flex:0 0 34rpx; justify-content:center; }.history-dot { position:absolute; z-index:2; top:38rpx; width:15rpx; height:15rpx; border:4rpx solid #dcd3f7; border-radius:50%; background:#7046e8; }.history-connector { position:absolute; top:49rpx; bottom:-43rpx; width:2rpx; background:#e5e0f1; }.history-copy { display:flex; min-width:0; flex:1; flex-direction:column; }.history-copy text:first-child { color:#3e3f4d; font-size:22rpx; font-weight:630; }.history-copy text:last-child { margin-top:5rpx; color:#9695a5; font-size:18rpx; }.history-order { color:#8b899a; font-size:18rpx; }.history-empty { display:flex; align-items:center; flex-direction:column; justify-content:center; min-height:260rpx; border:1rpx solid rgba(91,76,133,.1); border-radius:25rpx; background:rgba(255,255,255,.75); color:#9695a5; text-align:center; }.history-empty>text:nth-child(2) { margin:15rpx 0 6rpx; color:#4d4e5c; font-size:24rpx; font-weight:650; }.history-empty>text:last-child { font-size:18rpx; }.empty-check { width:38rpx; height:38rpx; border:5rpx solid #cfc9df; border-radius:50%; }
	.detail-title,.history-title { font-size:31rpx; }.detail-days-stat,.calendar-weekdays,.calendar-day,.history-count,.average-pill,.history-copy text:last-child,.history-order { font-size:18rpx; }.detail-stat-side text:first-child,.detail-stat-side text:last-child { font-size:24rpx; line-height:34rpx; }.detail-days-number { font-size:31rpx; line-height:38rpx; }.detail-days-unit { font-size:24rpx; line-height:34rpx; }.detail-no-record { font-size:24rpx; line-height:36rpx; }.calendar-header>text,.average-pill text:last-child,.history-copy text:first-child { font-size:24rpx; }
