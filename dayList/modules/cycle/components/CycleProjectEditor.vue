<template>
	<view v-if="visible" class="cycle-editor-mask" @tap="close">
		<view class="cycle-editor" @tap.stop>
			...
			<view class="cycle-icon-input-row"><input v-model="editorIcon" class="cycle-editor-input cycle-icon-input" maxlength="4" placeholder="如：🏃" /></view>
			...
			<view class="cycle-plan-row">
				<input v-model="editorIntervalValue" class="cycle-editor-input cycle-plan-input" type="number" maxlength="3" placeholder="非必填" @input="sanitizeInterval" />
...
<script>
	export default {
		...
		data() {
			return { editorTitle: '', editorIcon: '', editorViewMode: 'week', editorIntervalValue: '', editorIntervalUnit: 'day' }
		},
		computed: {
		...
		methods: {
			...
			save() {
				...
				this.$emit('save', { title: this.editorTitle.trim(), icon: this.editorIcon, viewMode: this.editorViewMode, intervalValue: this.editorIntervalValue, intervalUnit: this.editorIntervalUnit })
			}
		}
	}
</script>
...
<style>
	...
	.cycle-editor { width:100%; max-width:430px; padding:28rpx 27rpx 25rpx; border:1rpx solid rgba(111,70,232,.15); border-radius:31rpx; background:#fff; box-shadow:0 26rpx 80rpx rgba(35,27,65,.23); }
