<template>
	<view class="cycle-page-shell">
		<view class="cycle-page-content">
			<view v-if="viewMode === 'active'" class="cycle-topbar">
				...
				<view class="cycle-top-actions">
					...
					<button class="cycle-icon-button cycle-sort-button" :disabled="loading || activeCycles.length < 2" aria-label="调整周期项目顺序" @tap="openSortEditor"><view class="cycle-sort-glyph"><view></view><view></view><view></view></view></button>
					...
					<view v-for="cycle in activeCycles" :key="cycle._id" class="cycle-card">
						<view class="cycle-card-header">
							<view class="cycle-name-area" role="button" :aria-label="`查看${cycle.title}详情`" @tap="openDetail(cycle)">
								...
								<view class="cycle-title-copy"><text class="cycle-item-title">{{ cycle.title }}</text><text v-if="cycle.checkin_description" class="cycle-item-description">{{ cycle.checkin_description }}</text></view>
						...
						<view class="cycle-card-main">
							...
							<button class="cycle-check-button" :class="{ completed: isCompletedToday(cycle) }" :disabled="mutationBusy" @tap="toggleCompletion(cycle)">
								<view class="cycle-check-icon"><view v-if="isCompletedToday(cycle)" class="cycle-check-mark"></view><view v-else class="cycle-check-plus"></view></view>
						...
						<view class="cycle-stats">
							...
							<view class="cycle-stat-side right" :class="{ due: isExpectedDateDue(cycle, currentDate) }"><text>预计下次</text><text>{{ nextDateText(cycle) }}</text></view>
						</view>
					</view>
				</view>
			</template>
		...
		<view v-if="sortOpen" class="cycle-sort-mask" @tap="closeSortEditor">
			<view class="cycle-sort-panel" @tap.stop>
				<view class="cycle-sort-header"><view><text class="cycle-sort-title">调整项目顺序</text><text class="cycle-sort-hint">按住右侧手柄上下拖动</text></view><button aria-label="关闭排序" @tap="closeSortEditor">×</button></view>
				<view class="cycle-sort-list">
					<view v-for="(cycle, index) in sortDraft" :key="cycle._id" class="cycle-sort-row" :class="{ dragging: sortDraggingIndex === index }">
						<view class="cycle-sort-item-icon">{{ cycle.icon }}</view>
						<view class="cycle-sort-item-copy"><text>{{ cycle.title }}</text><text v-if="cycle.checkin_description">{{ cycle.checkin_description }}</text></view>
						<view class="cycle-sort-handle" role="button" :aria-label="`拖动${cycle.title}`" @touchstart.stop="startSortDrag(index, $event)" @touchmove.stop.prevent="moveSortDrag" @touchend.stop="endSortDrag" @touchcancel.stop="endSortDrag"><view></view><view></view><view></view></view>
					</view>
				</view>
				<view class="cycle-sort-actions"><button class="cycle-sort-cancel" :disabled="sortSaving" @tap="closeSortEditor">取消</button><button class="cycle-sort-save" :disabled="sortSaving" @tap="saveSortOrder">{{ sortSaving ? '保存中…' : '保存排序' }}</button></view>
			</view>
		</view>
...
<script>
	...
	import { daysBetween, expectedNextDate, formatFullDate, intervalLabel, isExpectedDateDue, lastCompletion, localDateString, monthDaysFor, parseLocalDate, weekDaysFor } from '../../modules/cycle/utils/cycle-date.js'
	...
	export default {
		...
		data() {
			return {
				...
				sortOpen: false, sortDraft: [], sortSaving: false, sortDraggingIndex: -1, sortDragOriginIndex: -1, sortDragStartY: 0
			}
		},
		computed: {
			...
			activeCycles() { return this.cycles.filter((cycle) => !cycle.archived) },
		...
		onBackPress() {
			if (this.sortOpen) { this.closeSortEditor(); return true }
		...
		methods: {
			...
			openSortEditor() {
				if (this.loading || this.activeCycles.length < 2) return
				this.sortDraft = this.activeCycles.map((cycle) => ({ _id: cycle._id, title: cycle.title, icon: cycle.icon, checkin_description: cycle.checkin_description || '' }))
				this.sortDraggingIndex = -1; this.sortDragOriginIndex = -1; this.sortOpen = true
			},
			closeSortEditor() {
				if (this.sortSaving) return
				this.sortOpen = false; this.sortDraft = []; this.endSortDrag()
			},
			startSortDrag(index, event) {
				if (this.sortSaving) return
				const y = this.sortTouchY(event)
				if (!Number.isFinite(y)) return
				this.sortDraggingIndex = index; this.sortDragOriginIndex = index; this.sortDragStartY = y
			},
			moveSortDrag(event) {
				if (this.sortDraggingIndex < 0 || this.sortDragOriginIndex < 0) return
				const y = this.sortTouchY(event)
				if (!Number.isFinite(y)) return
				const rowStep = typeof uni.upx2px === 'function' ? Math.max(1, uni.upx2px(92)) : 46
				const offset = Math.round((y - this.sortDragStartY) / rowStep)
				const target = Math.max(0, Math.min(this.sortDraft.length - 1, this.sortDragOriginIndex + offset))
				if (target === this.sortDraggingIndex) return
				const draft = this.sortDraft.slice()
				const [moving] = draft.splice(this.sortDraggingIndex, 1)
				draft.splice(target, 0, moving)
				this.sortDraft = draft; this.sortDraggingIndex = target
			},
			endSortDrag() { this.sortDraggingIndex = -1; this.sortDragOriginIndex = -1; this.sortDragStartY = 0 },
			sortTouchY(event) {
				const touch = event && event.touches && event.touches[0] ? event.touches[0] : (event && event.changedTouches && event.changedTouches[0])
				return touch ? Number(typeof touch.clientY === 'number' ? touch.clientY : touch.pageY) : NaN
			},
			async saveSortOrder() {
				if (!this.sortOpen || this.sortSaving) return
				this.sortSaving = true; this.endSortDrag()
				try {
					await cycleDataService.reorderActiveCycles({ ids: this.sortDraft.map((cycle) => cycle._id) })
					const result = await cycleDataService.listCycles(); this.cycles = result.cycles; this.sortOpen = false; this.sortDraft = []; showCycleFeedback('排序已保存')
				} catch (error) { showCycleFeedback(this.errorMessage(error, '排序保存失败')) }
				finally { this.sortSaving = false }
			},
...
<style>
	...
	.cycle-top-actions { display:flex; gap:13rpx; }.cycle-icon-button { display:flex; flex:0 0 72rpx; align-items:center; justify-content:center; width:72rpx; height:72rpx; margin:0; padding:0; border:1rpx solid rgba(85,73,130,.12); border-radius:50%; background:rgba(255,255,255,.82) !important; box-shadow:0 8rpx 22rpx rgba(77,65,120,.06); }.cycle-icon-button::after { border:0 !important; }.cycle-add-button { border-color:#7046e8; background:#7046e8 !important; }
	.cycle-sort-button[disabled] { opacity:.38; }.cycle-sort-glyph { display:flex; width:31rpx; height:27rpx; align-items:flex-start; flex-direction:column; justify-content:space-between; }.cycle-sort-glyph>view { height:4rpx; border-radius:4rpx; background:#454553; }.cycle-sort-glyph>view:first-child { width:31rpx; }.cycle-sort-glyph>view:nth-child(2) { width:23rpx; }.cycle-sort-glyph>view:last-child { width:15rpx; }
	...
	.cycle-stat-side.right.due { margin:-7rpx -9rpx -7rpx 0; padding:7rpx 9rpx; border-radius:15rpx; background:linear-gradient(100deg,rgba(255,244,226,.18),rgba(255,230,194,.82)); box-shadow:inset -4rpx 0 0 #ef5f16; }.cycle-stat-side.right.due text:first-child { color:#ef5f16; font-weight:700; }.cycle-stat-side.right.due text:last-child { color:#ef5f16; font-weight:780; }
	.cycle-sort-mask { position:fixed; z-index:1050; inset:0; display:flex; align-items:flex-end; justify-content:center; padding:30rpx 24rpx calc(30rpx + env(safe-area-inset-bottom)); background:rgba(28,25,42,.42); }.cycle-sort-panel { display:flex; overflow:hidden; width:100%; max-width:430px; max-height:calc(100vh - 90rpx - env(safe-area-inset-bottom)); flex-direction:column; padding:27rpx; border:1rpx solid rgba(111,70,232,.15); border-radius:31rpx; background:#fff; box-shadow:0 26rpx 80rpx rgba(35,27,65,.23); }.cycle-sort-header { display:flex; flex:0 0 auto; align-items:center; justify-content:space-between; margin-bottom:20rpx; }.cycle-sort-header>view { display:flex; min-width:0; flex-direction:column; }.cycle-sort-title { color:#292a38; font-size:29rpx; font-weight:730; }.cycle-sort-hint { margin-top:5rpx; color:#9291a2; font-size:24rpx; }.cycle-sort-header button { width:55rpx; height:55rpx; margin:0; padding:0 0 5rpx; border:0 !important; border-radius:50%; background:#f2f0f7 !important; color:#777687; font-size:38rpx; line-height:52rpx; }.cycle-sort-header button::after { border:0 !important; }.cycle-sort-list { display:flex; min-height:0; max-height:55vh; overflow-y:auto; flex-direction:column; gap:10rpx; padding:3rpx; }.cycle-sort-row { display:flex; flex:0 0 auto; min-height:82rpx; align-items:center; padding:9rpx 10rpx 9rpx 13rpx; border:1rpx solid rgba(91,76,133,.1); border-radius:18rpx; background:#faf9fc; transition:background .12s ease,box-shadow .12s ease,transform .12s ease; }.cycle-sort-row.dragging { z-index:2; border-color:rgba(111,70,232,.3); background:#f0ebff; box-shadow:0 10rpx 24rpx rgba(83,55,157,.15); transform:scale(1.012); }.cycle-sort-item-icon { display:flex; flex:0 0 52rpx; align-items:center; justify-content:center; width:52rpx; height:52rpx; margin-right:13rpx; border-radius:15rpx; background:#eee9fb; color:#7046e8; font-size:26rpx; }.cycle-sort-item-copy { display:flex; min-width:0; flex:1; flex-direction:column; }.cycle-sort-item-copy>text:first-child { overflow:hidden; color:#343543; font-size:24rpx; font-weight:680; text-overflow:ellipsis; white-space:nowrap; }.cycle-sort-item-copy>text:last-child:not(:first-child) { margin-top:2rpx; overflow:hidden; color:#9291a2; font-size:20rpx; text-overflow:ellipsis; white-space:nowrap; }.cycle-sort-handle { display:flex; flex:0 0 58rpx; align-items:center; flex-direction:column; justify-content:center; gap:6rpx; width:58rpx; height:58rpx; margin-left:10rpx; border-radius:15rpx; background:#f0edf5; touch-action:none; user-select:none; }.cycle-sort-handle>view { width:25rpx; height:3rpx; border-radius:3rpx; background:#777687; }.cycle-sort-actions { display:flex; flex:0 0 auto; gap:14rpx; margin-top:22rpx; }.cycle-sort-cancel,.cycle-sort-save { flex:1; height:72rpx; margin:0; padding:0; border:0 !important; border-radius:18rpx; font-size:24rpx; line-height:72rpx; }.cycle-sort-cancel { background:#f1f0f5 !important; color:#686878 !important; }.cycle-sort-save { background:#7046e8 !important; color:#fff !important; }.cycle-sort-cancel::after,.cycle-sort-save::after { border:0 !important; }.cycle-sort-cancel[disabled],.cycle-sort-save[disabled] { opacity:.5; }
	...
	@media (min-width:700px) { .cycle-page-shell { padding:35px 0; }.cycle-page-content { min-height:calc(100vh - 70px); border-radius:30px; background:rgba(250,249,253,.72); box-shadow:0 24px 80px rgba(54,42,92,.12); }.cycle-sort-mask { align-items:center; } }
</style>
